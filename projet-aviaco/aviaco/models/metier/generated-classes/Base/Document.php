...
abstract class Document implements ActiveRecordInterface
{
    ...
    /**
     * The value for the part_id_fk field.
     * @var        int
     */
    protected $part_id_fk;
    ...
    /**
     * @var        ChildPartenairepiece
     */
    protected $aPartenairepiece;
    ...
    /**
     * Get the [part_id_fk] column value.
     *
     * @return int
     */
    public function getPart_id_FK()
    {
        return $this->part_id_fk;
    }
     ...
     */
    public function getDateenreg_FK($format = NULL)
    {
        if ($format === null) {
    ...
    public function setReference_FK($v)
    {
        ...
        if ($this->reference_fk !== $v) {
            $this->reference_fk = $v;
            $this->modifiedColumns[DocumentTableMap::COL_REFERENCE_FK] = true;
        }
    ...
    /**
     * Set the value of [part_id_fk] column.
     *
     * @param int $v new value
     * @return $this|\Document The current object (for fluent API support)
     */
    public function setPart_id_FK($v)
    {
        if ($v !== null) {
            $v = (int) $v;
        }

        if ($this->part_id_fk !== $v) {
            $this->part_id_fk = $v;
            $this->modifiedColumns[DocumentTableMap::COL_PART_ID_FK] = true;
        }
     ...
     * @return $this|\Document The current object (for fluent API support)
     */
    public function setDateenreg_FK($v)
    {
        ...
        if ($this->aPartenairepiece !== null && $this->aPartenairepiece->getDateenregistrement() !== $v) {
            $this->aPartenairepiece = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : DocumentTableMap::translateFieldName('Reference_FK', TableMap::TYPE_PHPNAME, $indexType)];
            $this->reference_fk = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : DocumentTableMap::translateFieldName('Part_id_FK', TableMap::TYPE_PHPNAME, $indexType)];
            $this->part_id_fk = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : DocumentTableMap::translateFieldName('Dateenreg_FK', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aPartenaire !== null && $this->part_id_fk !== $this->aPartenaire->getPartenaire()) {
            $this->aPartenaire = null;
        }
        if ($this->aPartenairepiece !== null && $this->date_enreg_fk !== $this->aPartenairepiece->getDateenregistrement()) {
            $this->aPartenairepiece = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aPartenairepiece !== null) {
                if ($this->aPartenairepiece->isModified() || $this->aPartenairepiece->isNew()) {
                    $affectedRows += $this->aPartenairepiece->save($con);
                }
                $this->setPartenairepiece($this->aPartenairepiece);
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(DocumentTableMap::COL_REFERENCE_FK)) {
            $modifiedColumns[':p' . $index++]  = 'reference_FK';
        }
        if ($this->isColumnModified(DocumentTableMap::COL_PART_ID_FK)) {
            $modifiedColumns[':p' . $index++]  = 'part_id_FK';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'reference_FK':
                        ...
                        break;
                    case 'part_id_FK':
                        $stmt->bindValue($identifier, $this->part_id_fk, PDO::PARAM_INT);
                        break;
                    case 'date_enreg_FK':
    ...
    public function getByPosition($pos)
    {
        switch ($pos) {
            ...
            case 2:
                ...
                break;
            case 3:
                return $this->getPart_id_FK();
                break;
            case 4:
                return $this->getDateenreg_FK();
                break;
            default:
                return null;
                break;
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            ...
            $keys[1] => $this->getDoc(),
            $keys[2] => $this->getReference_FK(),
            $keys[3] => $this->getPart_id_FK(),
            $keys[4] => $this->getDateenreg_FK(),
        );
            ...
            if (null !== $this->aPartenairepiece) {
                ...
                switch ($keyType) {
                    ...
                    case TableMap::TYPE_FIELDNAME:
                        $key = 'partenaire_piece';
                        break;
                    default:
                        $key = 'Partenairepiece';
                }

                $result[$key] = $this->aPartenairepiece->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
            if (null !== $this->aPiece) {
    ...
    public function setByPosition($pos, $value)
    {
        switch ($pos) {
            ...
            case 2:
                ...
                break;
            case 3:
                $this->setPart_id_FK($value);
                break;
            case 4:
                $this->setDateenreg_FK($value);
                break;
        } // switch()
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setReference_FK($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setPart_id_FK($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setDateenreg_FK($arr[$keys[4]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(DocumentTableMap::COL_REFERENCE_FK)) {
            $criteria->add(DocumentTableMap::COL_REFERENCE_FK, $this->reference_fk);
        }
        if ($this->isColumnModified(DocumentTableMap::COL_PART_ID_FK)) {
            $criteria->add(DocumentTableMap::COL_PART_ID_FK, $this->part_id_fk);
        }
        if ($this->isColumnModified(DocumentTableMap::COL_DATE_ENREG_FK)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        $copyObj->setPart_id_FK($this->getPart_id_FK());
        $copyObj->setDateenreg_FK($this->getDateenreg_FK());
        if ($makeNew) {
            $copyObj->setNew(true);
    ...
    /**
     * Declares an association between this object and a ChildPartenairepiece object.
     *
     * @param  ChildPartenairepiece $v
     * @return $this|\Document The current object (for fluent API support)
     * @throws PropelException
     */
    public function setPartenairepiece(ChildPartenairepiece $v = null)
    {
        if ($v === null) {
            $this->setDateenreg_FK(NULL);
        } else {
            $this->setDateenreg_FK($v->getDateenregistrement());
        }

        $this->aPartenairepiece = $v;

        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildPartenairepiece object, it will not be re-added.
        if ($v !== null) {
            $v->addDoc($this);
        }
    ...
    /**
     * Get the associated ChildPartenairepiece object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildPartenairepiece The associated ChildPartenairepiece object.
     * @throws PropelException
     */
    public function getPartenairepiece(ConnectionInterface $con = null)
    {
        if ($this->aPartenairepiece === null && (($this->date_enreg_fk !== "" && $this->date_enreg_fk !== null))) {
            $this->aPartenairepiece = ChildPartenairepieceQuery::create()
                ->filterByDoc($this) // here
                ->findOne($con);
            /* The following can be used additionally to
                ...
                $this->aPartenairepiece->addDocs($this);
             */
        }

        return $this->aPartenairepiece;
    }
    ...
    {
        if ($v === null) {
            $this->setPart_id_FK(NULL);
        } else {
            $this->setPart_id_FK($v->getPartenaire());
        }
    ...
    {
        if ($this->aPartenaire === null && ($this->part_id_fk !== null)) {
            $this->aPartenaire = ChildPartenaireQuery::create()->findPk($this->part_id_fk, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aPartenaire->addDocs($this);
             */
        }
    ...
    {
        if (null !== $this->aPartenairepiece) {
            $this->aPartenairepiece->removeDoc($this);
        }
        if (null !== $this->aPiece) {
            $this->aPiece->removeDoc($this);
        }
        if (null !== $this->aPartenaire) {
            $this->aPartenaire->removeDoc($this);
        }
        $this->doc_num = null;
        $this->doc_lien = null;
        $this->reference_fk = null;
        $this->part_id_fk = null;
