...
/**
 * Base class that represents a row from the 'piece_appareil' table.
 *
 *
 *
* @package    propel.generator..Base
*/
abstract class PieceApp implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\PieceAppTableMap';
    ...
    /**
     * Initializes internal state of Base\PieceApp object.
     */
    ...
    /**
     * Compares this with another <code>PieceApp</code> instance.  If
     * <code>obj</code> is an instance of <code>PieceApp</code>, delegates to
     * <code>equals(PieceApp)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|PieceApp The current object, for fluid interface
     */
     ...
     * @return $this|\PieceApp The current object (for fluent API support)
     */
    public function setID($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[PieceAppTableMap::COL_ID] = true;
        }
    ...
    public function setIdAp_PK($v)
    {
        ...
        if ($this->idap_pk !== $v) {
            $this->idap_pk = $v;
            $this->modifiedColumns[PieceAppTableMap::COL_IDAP_PK] = true;
        }

        if ($this->aAppareil !== null && $this->aAppareil->getIdAppareil() !== $v) {
            $this->aAppareil = null;
        }
    ...
    public function setModele_PK($v)
    {
        ...
        if ($this->modele_pk !== $v) {
            $this->modele_pk = $v;
            $this->modifiedColumns[PieceAppTableMap::COL_MODELE_PK] = true;
        }
    ...
    public function setMarque_PK($v)
    {
        ...
        if ($this->marque_pk !== $v) {
            $this->marque_pk = $v;
            $this->modifiedColumns[PieceAppTableMap::COL_MARQUE_PK] = true;
        }
    ...
    public function setReference_PK($v)
    {
        ...
        if ($this->reference_pk !== $v) {
            $this->reference_pk = $v;
            $this->modifiedColumns[PieceAppTableMap::COL_REFERENCE_PK] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PieceAppTableMap::translateFieldName('ID', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PieceAppTableMap::translateFieldName('IdAp_PK', TableMap::TYPE_PHPNAME, $indexType)];
            $this->idap_pk = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PieceAppTableMap::translateFieldName('Modele_PK', TableMap::TYPE_PHPNAME, $indexType)];
            $this->modele_pk = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PieceAppTableMap::translateFieldName('Marque_PK', TableMap::TYPE_PHPNAME, $indexType)];
            $this->marque_pk = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : PieceAppTableMap::translateFieldName('Reference_PK', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = PieceAppTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\PieceApp'), 0, $e);
        }
    }
    ...
    {
        if ($this->aAppareil !== null && $this->idap_pk !== $this->aAppareil->getIdAppareil()) {
            $this->aAppareil = null;
        }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildPieceAppQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see PieceApp::setDeleted()
     * @see PieceApp::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildPieceAppQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                PieceAppTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[PieceAppTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . PieceAppTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(PieceAppTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_IDAP_PK)) {
            $modifiedColumns[':p' . $index++]  = 'idAp_PK';
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_MODELE_PK)) {
            $modifiedColumns[':p' . $index++]  = 'modele_PK';
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_MARQUE_PK)) {
            $modifiedColumns[':p' . $index++]  = 'marque_PK';
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_REFERENCE_PK)) {
            $modifiedColumns[':p' . $index++]  = 'reference_PK';
        }

        $sql = sprintf(
            'INSERT INTO piece_appareil (%s) VALUES (%s)',
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'id':
                        ...
                        break;
                    case 'idAp_PK':
                        $stmt->bindValue($identifier, $this->idap_pk, PDO::PARAM_STR);
                        break;
                    case 'modele_PK':
    ...
    {
        $pos = PieceAppTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['PieceApp'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['PieceApp'][$this->hashCode()] = true;
        $keys = PieceAppTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getID(),
            $keys[1] => $this->getIdAp_PK(),
            $keys[2] => $this->getModele_PK(),
            $keys[3] => $this->getMarque_PK(),
            $keys[4] => $this->getReference_PK(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[1], $arr)) {
            $this->setIdAp_PK($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setModele_PK($arr[$keys[2]]);
        }
        if (array_key_exists($keys[3], $arr)) {
            $this->setMarque_PK($arr[$keys[3]]);
        }
        if (array_key_exists($keys[4], $arr)) {
            $this->setReference_PK($arr[$keys[4]]);
        }
    }
    ...
    {
        $criteria = new Criteria(PieceAppTableMap::DATABASE_NAME);

        if ($this->isColumnModified(PieceAppTableMap::COL_ID)) {
            $criteria->add(PieceAppTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_IDAP_PK)) {
            $criteria->add(PieceAppTableMap::COL_IDAP_PK, $this->idap_pk);
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_MODELE_PK)) {
            $criteria->add(PieceAppTableMap::COL_MODELE_PK, $this->modele_pk);
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_MARQUE_PK)) {
            $criteria->add(PieceAppTableMap::COL_MARQUE_PK, $this->marque_pk);
        }
        if ($this->isColumnModified(PieceAppTableMap::COL_REFERENCE_PK)) {
            $criteria->add(PieceAppTableMap::COL_REFERENCE_PK, $this->reference_pk);
        }
    ...
    {
        $criteria = ChildPieceAppQuery::create();
        $criteria->add(PieceAppTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \PieceApp (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \PieceApp Clone of current object.
     * @throws PropelException
     */
    ...
    public function setAppareil(ChildAppareil $v = null)
    {
        if ($v === null) {
            ...
            $this->setIdAp_PK($v->getIdAppareil());
        }
    ...
    {
        if ($this->aAppareil === null && (($this->idap_pk !== "" && $this->idap_pk !== null))) {
            $this->aAppareil = ChildAppareilQuery::create()
                ->filterByPieceApp($this) // here
                ->findOne($con);
            /* The following can be used additionally to
                ...
                $this->aAppareil->addPieceApps($this);
             */
        }
    ...
    public function getModele(ConnectionInterface $con = null)
    {
        if ($this->aModele === null && (($this->modele_pk !== "" && $this->modele_pk !== null))) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aModele->addPieceApps($this);
             */
        }
    ...
    public function getMarque(ConnectionInterface $con = null)
    {
        if ($this->aMarque === null && (($this->marque_pk !== "" && $this->marque_pk !== null))) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aMarque->addPieceApps($this);
             */
        }
    ...
    {
        if (null !== $this->aAppareil) {
            $this->aAppareil->removePieceApp($this);
        }
        if (null !== $this->aModele) {
            $this->aModele->removePieceApp($this);
        }
        if (null !== $this->aMarque) {
            $this->aMarque->removePieceApp($this);
        }
        if (null !== $this->aPiece) {
            $this->aPiece->removePieceApp($this);
        }
        $this->id = null;
        $this->idap_pk = null;
        $this->modele_pk = null;
    ...
    {
        return (string) $this->exportTo(PieceAppTableMap::DEFAULT_STRING_FORMAT);
    }
