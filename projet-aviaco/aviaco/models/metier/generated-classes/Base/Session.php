...
abstract class Session implements ActiveRecordInterface
{
     ...
     */
    public function getDatesession($format = NULL)
    {
        if ($format === null) {
            return $this->date_connexion;
        } else {
            return $this->date_connexion instanceof \DateTime ? $this->date_connexion->format($format) : null;
        }
    }
    ...
    public function setIDS($v)
    {
        ...
        if ($this->ids !== $v) {
            $this->ids = $v;
            $this->modifiedColumns[SessionTableMap::COL_IDS] = true;
        }
     ...
     * @return $this|\Session The current object (for fluent API support)
     */
    public function setDatesession($v)
    {
        $dt = PropelDateTime::newInstance($v, null, 'DateTime');
        if ($this->date_connexion !== null || $dt !== null) {
            if ($this->date_connexion === null || $dt === null || $dt->format("Y-m-d H:i:s") !== $this->date_connexion->format("Y-m-d H:i:s")) {
                $this->date_connexion = $dt === null ? null : clone $dt;
                $this->modifiedColumns[SessionTableMap::COL_DATE_CONNEXION] = true;
            }
    ...
    public function setId_emp_FK($v)
    {
        ...
        if ($this->id_emp_fk !== $v) {
            $this->id_emp_fk = $v;
            $this->modifiedColumns[SessionTableMap::COL_ID_EMP_FK] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : SessionTableMap::translateFieldName('IDS', TableMap::TYPE_PHPNAME, $indexType)];
            $this->ids = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : SessionTableMap::translateFieldName('Datesession', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00 00:00:00') {
                $col = null;
            }
            $this->date_connexion = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : SessionTableMap::translateFieldName('Id_emp_FK', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(SessionTableMap::COL_IDS)) {
            $modifiedColumns[':p' . $index++]  = 'IDS';
        }
        if ($this->isColumnModified(SessionTableMap::COL_DATE_CONNEXION)) {
            $modifiedColumns[':p' . $index++]  = 'date_connexion';
        }
        if ($this->isColumnModified(SessionTableMap::COL_ID_EMP_FK)) {
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'IDS':
                        $stmt->bindValue($identifier, $this->ids, PDO::PARAM_INT);
                        break;
                    case 'date_connexion':
                        $stmt->bindValue($identifier, $this->date_connexion ? $this->date_connexion->format("Y-m-d H:i:s") : null, PDO::PARAM_STR);
                        break;
                    case 'id_emp_FK':
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        $result = array(
            $keys[0] => $this->getIDS(),
            $keys[1] => $this->getDatesession(),
            $keys[2] => $this->getId_emp_FK(),
        );
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[0], $arr)) {
            $this->setIDS($arr[$keys[0]]);
        }
        if (array_key_exists($keys[1], $arr)) {
            $this->setDatesession($arr[$keys[1]]);
        }
        if (array_key_exists($keys[2], $arr)) {
            $this->setId_emp_FK($arr[$keys[2]]);
        }
    }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(SessionTableMap::COL_IDS)) {
            $criteria->add(SessionTableMap::COL_IDS, $this->ids);
        }
        if ($this->isColumnModified(SessionTableMap::COL_DATE_CONNEXION)) {
            $criteria->add(SessionTableMap::COL_DATE_CONNEXION, $this->date_connexion);
        }
        if ($this->isColumnModified(SessionTableMap::COL_ID_EMP_FK)) {
            $criteria->add(SessionTableMap::COL_ID_EMP_FK, $this->id_emp_fk);
        }
    ...
    /**
     * Generic method to set the primary key (ids column).
     *
    ...
    {
        $copyObj->setDatesession($this->getDatesession());
        $copyObj->setId_emp_FK($this->getId_emp_FK());
        if ($makeNew) {
            $copyObj->setNew(true);
            $copyObj->setIDS(NULL); // this is a auto-increment column, so set to default value
        }
    }
    ...
    public function getEmploye(ConnectionInterface $con = null)
    {
        if ($this->aEmploye === null && (($this->id_emp_fk !== "" && $this->id_emp_fk !== null))) {
            ...
            /* The following can be used additionally to
                ...
                $this->aEmploye->addSessions($this);
             */
        }
    ...
    {
        if (null !== $this->aEmploye) {
            $this->aEmploye->removeSession($this);
        }
        $this->ids = null;
