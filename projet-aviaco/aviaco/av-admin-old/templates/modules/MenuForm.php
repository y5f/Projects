...
class MenuForm {
    ...
    function __construct() {
        ...
        foreach ($mes_cats as $cat){
            ...
            $c_box->setAttribute('value', $cat->getURL());
            $em=new Em();
        ...
        $b_select->addElements(Array($s_titre,$bloc_menu,$bloc_cat,$bloc_art));
        ...
        $b_prop->addElements(Array($prop_titre,$prop_input_id,$prop_p_p,$prop_p_desc,$prop_p_desc_input,$prop_p_url,$prop_p_url_input,$prop_p_ord,$prop_p_ord_input,$prop_p_com,$prop_p_com_input,$prop_p_bout));
    ...
    function getSousCategorie(Categorie $cat){
        ...
        foreach ($cat->getSouscategories() as $s_cat){
            ...
            $c_box->setAttribute('type', 'checkbox');
            $c_box->setAttribute('class', 'cbx');
            $c_box->setAttribute('value', $s_cat->getURL());
            $em=new Em();
            ...
            $s_categ=CategorieQuery::create()->filterByCategorie($s_cat->getSouscategorie())->findOne();
            if($s_categ && $s_categ->getNiveau()!=$cat->getNiveau()){
                $li->addElement($this->getSousCategorie($s_categ));
            }
            $ul->addElement($li);   
        }
        return $ul;
    ...
    function getSousMenu(Menus $mn,$bool){
        ...
        foreach (SousmenuQuery::create()->filterByMenus($mn)->orderByORDRE('ASC')->find() as $s_mn){
            ...
            // on vérifie si la catégorie à des enfants //
            $s_categ=MenusQuery::create()->filterByMenu($s_mn->getSousmenu())->findOne();
            if($s_categ && $s_categ->getNIVEAU()!=$mn->getNIVEAU()){
                $li->addElement($this->getSousMenu($s_categ,$bool));
            }
            $ul->addElement($li); 
        }
        return $ul;
    ...
    function getMenus($bool){
        ...
        foreach (MenusQuery::create()->filterByNiveau(1)->orderByOrdre('ASC')->find() as $mn) {
            if($bool){
                ...
                $c_box->setAttribute('type', 'checkbox');
                $c_box->setAttribute('class', 'cbx');
                $c_box->setAttribute('value', $mn->getURL());
                $em=new Em();
