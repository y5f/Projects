...
function load(){$(document).ready(function(){
    ...
    */
    ...
    function refreshdata(resultat){
        ...
        str+='<option value='+ tab[0] +'>'+tab[2]+'</option>';
    ...
    $('.ad-a').click(function(){
        var ele=$('.cbx:checked');
        $('#desc').val(ele.attr('name'));
        $('#url').val(ele.attr('value'));
    });
    $('.cbx').click(function(){
        if(idx_control){
            ...
            $('.cbx').each(function(){
                if($(this).prop('checked')){
                    ...
                    $.ajax({
                        type:'POST',
                        url:'scripts/script.php',
                        data:tab+'=' + $(this).prop('value') + '^' + $(this).attr('name'),
                        success:refreshdata
                    });
                }
            });
...
function load_search(rslt){
    $('.b-list-widget').html(rslt);
    init();
}
function init(){
    //############### SCRIPT POUR LES WIDGETS ######################
...
function hover(e,ui){
        $('.b-pann').css({
            'border':'dashed #9d9d9d 0.1em'
        });
        $(this).css({
            'border':'solid #090 0.1em'
        });
    }
    function dragg(e,ui){
        var num_art=$(ui.draggable).attr('value');
        var num_bloc=$(this).attr('id');
        //alert(num_art + ' ' + num_bloc);
        $.ajax({
            type:'POST',
            url:'scripts/script.php',
            data:'load-panneau=' + num_art + '^' + num_bloc,
            success:load_widget
        });
        $('.b-pann').css({
            'border':'dashed #9d9d9d 0.1em'
