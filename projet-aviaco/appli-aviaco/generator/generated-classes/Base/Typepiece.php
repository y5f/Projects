...
/**
 * Base class that represents a row from the 'typepiece' table.
 *
 *
 *
* @package    propel.generator..Base
*/
abstract class Typepiece implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\TypepieceTableMap';
     ...
     */
    protected $collTypepieces;
    protected $collTypepiecesPartial;
     ...
     */
    protected $typepiecesScheduledForDeletion = null;
    ...
    /**
     * Initializes internal state of Base\Typepiece object.
     */
    ...
    /**
     * Compares this with another <code>Typepiece</code> instance.  If
     * <code>obj</code> is an instance of <code>Typepiece</code>, delegates to
     * <code>equals(Typepiece)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Typepiece The current object, for fluid interface
     */
     ...
     * @return $this|\Typepiece The current object (for fluent API support)
     */
    public function setID($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[TypepieceTableMap::COL_ID] = true;
        }
    ...
    public function setType($v)
    {
        ...
        if ($this->type !== $v) {
            $this->type = $v;
            $this->modifiedColumns[TypepieceTableMap::COL_TYPE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : TypepieceTableMap::translateFieldName('ID', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : TypepieceTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = TypepieceTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Typepiece'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildTypepieceQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Typepiece::setDeleted()
     * @see Typepiece::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildTypepieceQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                TypepieceTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->typepiecesScheduledForDeletion !== null) {
                if (!$this->typepiecesScheduledForDeletion->isEmpty()) {
                    \PieceQuery::create()
                        ->filterByPrimaryKeys($this->typepiecesScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->typepiecesScheduledForDeletion = null;
                }
            }

            if ($this->collTypepieces !== null) {
                foreach ($this->collTypepieces as $referrerFK) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[TypepieceTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . TypepieceTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(TypepieceTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(TypepieceTableMap::COL_TYPE)) {
            $modifiedColumns[':p' . $index++]  = 'type';
        }

        $sql = sprintf(
            'INSERT INTO typepiece (%s) VALUES (%s)',
    ...
    {
        $pos = TypepieceTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Typepiece'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Typepiece'][$this->hashCode()] = true;
        $keys = TypepieceTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getID(),
            $keys[1] => $this->getType(),
        );
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collTypepieces) {
                ...
                $result[$key] = $this->collTypepieces->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
    ...
    {
        $criteria = new Criteria(TypepieceTableMap::DATABASE_NAME);

        if ($this->isColumnModified(TypepieceTableMap::COL_ID)) {
            $criteria->add(TypepieceTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(TypepieceTableMap::COL_TYPE)) {
            $criteria->add(TypepieceTableMap::COL_TYPE, $this->type);
        }
    ...
    {
        $criteria = ChildTypepieceQuery::create();
        $criteria->add(TypepieceTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Typepiece (or compatible) type.
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getTypepieces() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addTypepiece($relObj->copy($deepCopy));
                }
            }
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Typepiece Clone of current object.
     * @throws PropelException
     */
    ...
    {
        if ('Typepiece' == $relationName) {
            return $this->initTypepieces();
        }
    ...
    /**
     * Clears out the collTypepieces collection
     *
     ...
     * @see        addTypepieces()
     */
    public function clearTypepieces()
    {
        $this->collTypepieces = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collTypepieces collection loaded partially.
     */
    public function resetPartialTypepieces($v = true)
    {
        $this->collTypepiecesPartial = $v;
    }

    /**
     * Initializes the collTypepieces collection.
     *
     * By default this just sets the collTypepieces collection to an empty array (like clearcollTypepieces());
     ...
     * @return void
     */
    public function initTypepieces($overrideExisting = true)
    {
        if (null !== $this->collTypepieces && !$overrideExisting) {
            return;
        }
        $this->collTypepieces = new ObjectCollection();
        $this->collTypepieces->setModel('\Piece');
    }
     ...
     * If this ChildTypepiece is new, it will return
     ...
     * @throws PropelException
     */
    public function getTypepieces(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collTypepiecesPartial && !$this->isNew();
        if (null === $this->collTypepieces || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collTypepieces) {
                // return empty collection
                $this->initTypepieces();
            } else {
                $collTypepieces = ChildPieceQuery::create(null, $criteria)
                    ->filterByTypepiece($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collTypepiecesPartial && count($collTypepieces)) {
                        $this->initTypepieces(false);

                        foreach ($collTypepieces as $obj) {
                            if (false == $this->collTypepieces->contains($obj)) {
                                $this->collTypepieces->append($obj);
                            }
                        }

                        $this->collTypepiecesPartial = true;
                    }

                    return $collTypepieces;
                }

                if ($partial && $this->collTypepieces) {
                    foreach ($this->collTypepieces as $obj) {
                        if ($obj->isNew()) {
                            $collTypepieces[] = $obj;
                        }
                    }
                ...
                $this->collTypepieces = $collTypepieces;
                $this->collTypepiecesPartial = false;
            }
        }

        return $this->collTypepieces;
    }
     ...
     *
     * @param      Collection $typepieces A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildTypepiece The current object (for fluent API support)
     */
    public function setTypepieces(Collection $typepieces, ConnectionInterface $con = null)
    {
        /** @var ChildPiece[] $typepiecesToDelete */
        $typepiecesToDelete = $this->getTypepieces(new Criteria(), $con)->diff($typepieces);


        $this->typepiecesScheduledForDeletion = $typepiecesToDelete;

        foreach ($typepiecesToDelete as $typepieceRemoved) {
            $typepieceRemoved->setTypepiece(null);
        }

        $this->collTypepieces = null;
        foreach ($typepieces as $typepiece) {
            $this->addTypepiece($typepiece);
        }

        $this->collTypepieces = $typepieces;
        $this->collTypepiecesPartial = false;
     ...
     * @throws PropelException
     */
    public function countTypepieces(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collTypepiecesPartial && !$this->isNew();
        if (null === $this->collTypepieces || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collTypepieces) {
            ...
            if ($partial && !$criteria) {
                return count($this->getTypepieces());
            }
        ...
        return count($this->collTypepieces);
    }
     ...
     * @return $this|\Typepiece The current object (for fluent API support)
     */
    public function addTypepiece(ChildPiece $l)
    {
        if ($this->collTypepieces === null) {
            $this->initTypepieces();
            $this->collTypepiecesPartial = true;
        ...
        if (!$this->collTypepieces->contains($l)) {
            $this->doAddTypepiece($l);
        }
    ...
    /**
     * @param ChildPiece $typepiece The ChildPiece object to add.
     */
    protected function doAddTypepiece(ChildPiece $typepiece)
    {
        $this->collTypepieces[]= $typepiece;
        $typepiece->setTypepiece($this);
    }

    /**
     * @param  ChildPiece $typepiece The ChildPiece object to remove.
     * @return $this|ChildTypepiece The current object (for fluent API support)
     */
    public function removeTypepiece(ChildPiece $typepiece)
    {
        if ($this->getTypepieces()->contains($typepiece)) {
            $pos = $this->collTypepieces->search($typepiece);
            $this->collTypepieces->remove($pos);
            if (null === $this->typepiecesScheduledForDeletion) {
                $this->typepiecesScheduledForDeletion = clone $this->collTypepieces;
                $this->typepiecesScheduledForDeletion->clear();
            }
            $this->typepiecesScheduledForDeletion[]= $typepiece;
            $typepiece->setTypepiece(null);
        }
    ...
    public function setSocietetypepieces(Collection $societetypepieces, ConnectionInterface $con = null)
    {
        ...
        foreach ($societetypepiecesToDelete as $societetypepieceRemoved) {
            $societetypepieceRemoved->setTypepiece(null);
        }
    ...
    protected function doAddSocietetypepiece(ChildSocietetypepiece $societetypepiece)
    {
        ...
        $societetypepiece->setTypepiece($this);
    }
    ...
    public function removeSocietetypepiece(ChildSocietetypepiece $societetypepiece)
    {
        if ($this->getSocietetypepieces()->contains($societetypepiece)) {
            ...
            $societetypepiece->setTypepiece(null);
        }
     ...
     * Otherwise if this Typepiece is new, it will return
     * an empty collection; or if this Typepiece has previously
     * been saved, it will retrieve related Societetypepieces from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Typepiece.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @return ObjectCollection|ChildSocietetypepiece[] List of ChildSocietetypepiece objects
     */
    public function getSocietetypepiecesJoinSociete(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildSocietetypepieceQuery::create(null, $criteria);
    ...
    {
        if ($deep) {
            if ($this->collTypepieces) {
                foreach ($this->collTypepieces as $o) {
    ...
    {
        return (string) $this->exportTo(TypepieceTableMap::DEFAULT_STRING_FORMAT);
    }
